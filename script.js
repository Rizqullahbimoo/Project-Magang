  ...
  async function runApp() {
    ...
    const state = {
      dataMaster: {
        ...
        proyek: '', jenisPerjanjian: '', metodePembayaran: '',
        disusunJabatan: '', disusunNama: '', mengetahuiJabatan: '', mengetahuiNama: '',
    ...
    function processMailMerge(text) {
      ...
      const map = {
        ...
        'Pengalaman Pekerjaan Sejenis': state.dataMaster.pengalamanPekerjaan,
    ...
    async function renderFullDocument() {
      ...
      DOM.previewPanel.innerHTML = '<div style="text-align:center; padding: 40px;">Membuat pratinjau dokumen...</div>';
      ...
      const coverHtml = createPageHtml(`
        <div class="cover-page" style="text-align: center;">
          <p style="margin-top: 10px;">${(state.dataMaster.judulPengadaan || '[JUDUL PENGADAAN]').toUpperCase()}</p>
          ...
          <div class="cover-bab-list" style="margin-top: 10px;">
            ${Object.entries(pageTemplates).map(([k, v]) => `<p>${k} – ${v.title.toUpperCase()}</p>`).join('')}
          </div>
          <div style="margin-top: 40px;">${(state.dataMaster.bulanRks || '[Bulan]').toUpperCase()} ${state.dataMaster.tahunRks || '[Tahun]'}</div>
        </div>`);

      const tocHtml = createPageHtml(`
        <h2 class="toc-title">DAFTAR ISI</h2>
        ${Object.entries(pageTemplates).map(([k, v], i) =>
          `<div class="toc-entry"><span>${k} ${v.title.toUpperCase()}</span><span>${i + 3}</span></div>`
        ).join('')}`);
      ...
      const pageContentMeasureEl = measureDiv.querySelector('.page-content-after-letterhead');
      
      const letterheadImg = measureDiv.querySelector('.letterhead-logo img');
      if (letterheadImg && !letterheadImg.complete) {
        await new Promise(resolve => { letterheadImg.onload = resolve; letterheadImg.onerror = resolve; });
      }
      const availableHeight = measureDiv.clientHeight - pageContentMeasureEl.offsetTop - 40; // Beri jarak 40px di bawah
    ...
    function updateState() {
      for (const k in state.dataMaster) if (DOM[k]) state.dataMaster[k] = DOM[k].value;
    }
    function attachForm() {
      if (DOM.form) DOM.form.addEventListener('input', () => { updateState(); renderFullDocument(); });
    }
    ...
    function initDownload() {
      ...
      form.addEventListener('submit', e => {
        ...
        const type = form.querySelector('input[name="file-type"]:checked').value;
        const content = DOM.previewPanel.innerHTML;
        if (type === 'docx') {
          const blob = window.htmlDocx.asBlob(content);
          const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${fileName}.docx`; link.click();
        } else {
          const opt = { margin: 0, filename: `${fileName}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
